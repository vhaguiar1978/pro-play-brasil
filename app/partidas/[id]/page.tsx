...
import { MatchBetSlip } from "@/components/matches/match-bet-slip";
...
export default function PartidaPage({ params }: Props) {
  ...
  const fetchMatch = useCallback(async () => {
    try {
      ...
      const found = (data.matches ?? []).find((item: Match) => item.id === id);
      setMatch(found ?? null);
    } catch {
      setMatch(null);
    }
  ...
  useEffect(() => {
    ...
    setMyNickname(profile?.gamertag ?? "");
  }, [fetchMatch]);
  ...
  function handleProofPick(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  async function uploadProofIfAny(): Promise<string | null> {
    ...
    try {
      ...
      const response = await fetch(`/api/matches/${match.id}/proof`, {
        method: "POST",
        body: formData
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Upload do print falhou");
  ...
  const canConfirm = match.status === "result_submitted" && iAmPlayer && match.submittedBy !== mySlot;
  ...
  async function submitResult() {
    ...
    try {
      ...
      await action(
        ...
        "Resultado enviado e aguardando confirmação do adversário"
      );
      setProofFile(null);
  ...
  function confirmResult() {
    ...
    action(
      ...
      undefined,
      "Resultado confirmado. O vencedor avançou no chaveamento."
    );
  }
  ...
  async function submitDispute() {
    ...
    try {
      ...
      await action(
        ...
        "Disputa registrada. O admin vai revisar."
      );
  ...
  function whatsappForOpponent(): string | null {
    ...
    const url =
      typeof window !== "undefined"
        ...
        : getAbsoluteSiteUrl(`/partidas/${match.id}`);
    const msg = `Salve ${opponent.nickname}! Aqui é ${
      ...
      match.status === "result_submitted"
        ? `Lancei o placar ${match.submittedScoreA ?? myScore} x ${match.submittedScoreB ?? opScore} para confirmação.`
  ...
  const statusGuide = [
    {
      label: "Jogar a partida",
      detail: "combine o confronto e resolva no jogo",
      tone: "primary" as const
    },
    {
      label: "Enviar o placar",
      detail: "suba o resultado com print se precisar",
      tone: "accent" as const
    },
    {
      label: "Confirmar ou contestar",
      detail: "o adversário confirma ou o admin revisa",
      tone: "gold" as const
    }
  ];
  ...
  return (
    <div className="flex flex-col gap-8 pb-20 md:pb-24">
      <section className="relative isolate overflow-hidden border-b border-ppb-border">
        ...
        <div className="mx-auto w-full max-w-6xl px-4 pb-10 pt-8 md:px-6 md:pb-12 md:pt-12">
          <Link
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1.04fr,0.96fr]">
            <div className="flex flex-col items-start gap-4">
              ...
              <div className="grid w-full max-w-3xl gap-3 sm:grid-cols-3">
                <HeroTag
                  ...
                  label="Confronto real"
                  text="aqui é onde a partida vira resultado oficial"
                  tone="primary"
                />
                <HeroTag
                  icon={<ShieldAlert className="h-4 w-4" />}
                  label="Segurança"
                  text="confirmação dupla e disputa quando o placar diverge"
                  tone="accent"
                />
                <HeroTag
                  icon={<Crown className="h-4 w-4" />}
                  label="Escalada"
                  text="cada confirmação empurra o jogador para o próximo passo"
                  tone="gold"
                />
              </div>
            </div>
            ...
            <div className="rounded-[28px] border border-white/10 bg-white/[0.05] p-5 shadow-[0_30px_80px_rgba(0,0,0,0.38)] backdrop-blur">
              ...
              <div className="mt-5 rounded-2xl border border-ppb-border bg-ppb-surface/70 p-4">
                ...
                <p className="mt-3 text-sm leading-6 text-ppb-muted">
                  Deixar o confronto claro, reduzir erro no placar e fazer o jogador sentir que resolver a partida aqui é simples e seguro.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 md:px-6">
            ...
            <div className="mt-6 space-y-4">
              {canSubmit ? (
                <div className="rounded-2xl border border-ppb-border bg-ppb-subtle/40 p-4">
                  ...
                  >
                    {loading || uploadingProof ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                    {uploadingProof
                      ? "Enviando print..."
                      : match.status === "result_submitted"
                        ? "Corrigir placar"
              ...
              {showDispute && canDispute ? (
                <div className="rounded-2xl border border-rose-500/40 bg-rose-500/5 p-4">
                  ...
                  <div className="mt-3 flex items-center justify-end gap-2">
                    ...
                    >
                      {loading || uploadingProof ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />}
                      Enviar contestação
                    </button>
                  </div>
                </div>
          ...
          {!iAmPlayer && match.status !== "finalized" && match.status !== "bye" && match.status !== "tbd" ? (
            <div className="mt-6 rounded-2xl border border-ppb-border bg-ppb-subtle/40 p-4 text-center">
              <p className="text-xs text-ppb-muted">
                Só os jogadores dessa partida podem enviar ou confirmar resultado.
                {!myNickname ? (
                  <>
                    {" "}
        ...
        {match.playerA && match.playerB ? (
          <div className="mt-6">
            <MatchBetSlip
              matchId={match.id}
              playerAName={match.playerA.nickname}
              playerBName={match.playerB.nickname}
              matchStatus={match.status}
              iAmPlayer={iAmPlayer}
              hasNickname={!!myNickname}
            />
          </div>
        ) : null}
      </section>
    ...
    <div className="mt-3">
      {proofPreview ? (
        <div className="relative overflow-hidden rounded-xl border border-ppb-border">
          ...
          <img src={proofPreview} alt="Preview do print" className="max-h-48 w-full bg-ppb-background object-contain" />
          <button
            type="button"
            onClick={onClear}
  ...
  if (!href) {
    return (
      <div className="flex items-start gap-2 rounded-xl border border-ppb-border bg-ppb-subtle/40 px-3 py-2.5 text-xs text-ppb-muted">
        ...
        <span>Adversário sem WhatsApp cadastrado. Avise por outro canal para ele confirmar o resultado.</span>
      </div>
    );
  }
...
function FlowPill({
  ...
  const toneClass = {
    primary: "border-ppb-primary/30 bg-ppb-primary/10 text-ppb-primary",
    accent: "border-ppb-accent/30 bg-ppb-accent/10 text-ppb-accent",
    gold: "border-ppb-gold/30 bg-ppb-gold/10 text-ppb-gold"
  }[tone];
  ...
  return (
    <div className="grid grid-cols-[auto,1fr] items-center gap-3 rounded-2xl border border-ppb-border bg-ppb-surface/60 px-4 py-3">
      <div className={cn("grid h-10 w-10 place-items-center rounded-2xl border text-sm font-black", toneClass)}>
        {index}
      </div>
      <div>
